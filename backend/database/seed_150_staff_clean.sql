-- Seed 150 Staff Members - Clean Version
-- Clears existing users first, then populates with 150 staff
-- Password for all: "password123" (bcrypt hash)

BEGIN;

-- Clear existing data (except keep HR if needed)
...
SELECT 
    gen_random_uuid(),
    'fm.' || LOWER(REPLACE(REPLACE(branch_name, 'Ace Mall, ', ''), ' ', '')) || '.' || LOWER(SUBSTRING(dept_name, 1, 2)) || rn::TEXT || '@acesupermarket.com',
    '$2a$10$ElkYaw5rar.b92GC6Fp/oeuyQmYleDeQMPhpaO4ngs5HFdlTnSRwS',
    CASE 
...
-- Cashiers (40 staff - 3 per branch)
...
SELECT 
    gen_random_uuid(),
    'waiter' || global_rn || '.' || LOWER(REPLACE(branch_name, 'Ace ', '')) || '@acesupermarket.com',
    '$2a$10$ElkYaw5rar.b92GC6Fp/oeuyQmYleDeQMPhpaO4ngs5HFdlTnSRwS',
    CASE 
